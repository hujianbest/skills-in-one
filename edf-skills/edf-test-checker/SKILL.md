---
name: edf-test-checker
description: Use when edf-tdd-implementation has finished and the freshly written tests must be independently audited for coverage / effectiveness / maintainability before code review, when a reviewer subagent is dispatched to perform post-TDD test effectiveness review, or when test-check returned 需修改 in a previous round. Not for writing or fixing tests (→ edf-tdd-implementation), not for reviewing production code (→ edf-code-review), not for stage / route confusion (→ edf-workflow-router).
---

# edf TDD 后测试有效性审查

独立审查 `edf-tdd-implementation` 已落地的测试用例是否真正有效：覆盖 AR 关键行为 / 边界 / 异常路径 / 嵌入式风险，断言能证明行为而不是仅证明代码被调用，可执行 / 稳定 / 可维护。

本 skill **不**补写测试、**不**修改生产代码、**不**替开发负责人决定优先级。它产出 verdict + findings + 唯一下一步。

edf-soul 要求：「TDD 中写出的测试用例不能天然视为有效，必须经过独立 test-check record。」本 skill 是这条纪律的工程化执行。

## When to Use

适用：

- `edf-tdd-implementation` 已写回 fresh evidence + 实现交接块，需独立审查测试有效性
- reviewer subagent 被派发执行 test-check
- 用户明确要求「review 测试 / test check / 测试有效性审查」

不适用 → 改用：

- 写 / 修测试 → `edf-tdd-implementation`
- 评审 C / C++ 代码质量 → `edf-code-review`
- 阶段不清 / 证据冲突 → `edf-workflow-router`

## Hard Gates

- `edf-test-checker` 通过前不得进入 `edf-code-review`
- reviewer **不**修改测试代码 / 生产代码 / AR 设计
- reviewer **不**补写测试用例（缺测试用例 → 视为 finding 让 `edf-tdd-implementation` 回修）
- reviewer **不**替开发负责人 / 模块架构师拍板
- reviewer **不**返回多个候选下一步
- 缺实现交接块或 fresh evidence → blocked-content，下一步 `edf-tdd-implementation`
- route / stage / profile 冲突 → blocked-workflow，`reroute_via_router=true`，下一步 `edf-workflow-router`
...
- 缺关键测试资产或实现交接块 → blocked-content，下一步 `edf-tdd-implementation`
- route / stage / profile 冲突 → blocked-workflow，`reroute_via_router=true`，下一步 `edf-workflow-router`
- 否则进入步骤 2
...
按 Checklist-Based Review + Mutation Mindset 跑 Group TC1-TC7 子规则（详见 `references/test-check-rubric.md`）。每条 finding 带 `severity` / `classification`（USER-INPUT / LLM-FIXABLE / TEAM-EXPERT） / `rule_id` / `anchor` / 描述 / 建议修复。Mock Boundary 维度按 `edf-ar-design/references/test-design-section-contract.md` 的 mock 规则核对；嵌入式风险矩阵按 `edf-tdd-implementation/references/embedded-evidence-checklist.md` 反向核对实测覆盖。
...
|---|---|---|---|
| 7 维度均 ≥ 6、嵌入式风险矩阵被实测覆盖、断言强度足够、无 critical USER-INPUT | `通过` | `edf-code-review` | `false` |
| findings 可 1-2 轮定向修订 | `需修改` | `edf-tdd-implementation` | `false` |
| 测试过于薄弱 / 核心行为未覆盖 / 嵌入式风险大量未覆盖 / findings 无法定向回修 | `阻塞`（内容） | `edf-tdd-implementation` | `false` |
| route / stage / profile / 上游证据冲突 | `阻塞`（workflow） | `edf-workflow-router` | `true` |

### 5. 写 review 记录并回传

按 `templates/edf-review-record-template.md` 写 `features/<id>/reviews/test-check.md`，并按 `edf-workflow-router/references/reviewer-dispatch-protocol.md` 回传结构化摘要。reviewer **不**补写测试 / **不**修改生产代码 / **不**返回多个候选下一步。
...
| `edf-ar-design/references/test-design-section-contract.md` | 测试设计章节契约（用例最小字段） |
| `edf-tdd-implementation/references/embedded-evidence-checklist.md` | 嵌入式证据落点 |
