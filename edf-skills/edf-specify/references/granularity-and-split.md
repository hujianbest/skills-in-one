# edf Granularity And Split

> 配套 `edf-specify/SKILL.md`。规定 `features/<id>/requirement.md` 的需求条目过大时如何检测、如何拆分；以及 SR work item 如何把候选 AR 拆出粒度合适。
>
> edf 不维护跨工作项的 deferred backlog——候选拆分的去向是「拆出新 AR work item」（实现子街区）或「写入 SR 的 AR Breakdown Candidates 章节」（需求分析子街区），由需求负责人决定何时新建。
...
edf 不维护 `spec-deferred.md` 等跨工作项 backlog。当一条 row 拆分后构成独立可发布能力时：
...
如果 SR 的所有候选 AR 全部「太粗」或「太细」，spec-review 的 `S7-SR AR Breakdown Candidates` 维度应给低分，回 `edf-specify` 重拆。
...
- 在 `edf-finalize` analysis closeout 时这条声明仍要保留在 `closeout.md`
...
- spec-review 中对粒度的 rubric 检查（含 G1-G6 与 SR 候选 AR 拆分质量）：`../edf-spec-review/references/spec-review-rubric.md`
