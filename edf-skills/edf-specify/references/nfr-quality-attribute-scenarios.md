# edf NFR Quality Attribute Scenarios（NFR QAS）

> 配套 `edf-specify/SKILL.md`。规定 `features/<id>/requirement.md` 中**每条核心 NFR** 的最小契约：以 ISO/IEC 25010 作为质量模型分类，用 Quality Attribute Scenarios（QAS）格式表达每条 NFR，让它在 spec 层面就具备可验证、可追溯的形状。
>
...
下表按 edf 主场景的常见相关性排序；嵌入式 / 实时 / 控制系统的 NFR 大多落在前 4 行。
...
- spec-review 对核心 NFR 的检查（必须有 QAS、Response Measure 阈值、Acceptance 与 QAS 一致）：`../edf-spec-review/references/spec-review-rubric.md`
...
- `edf-ar-design` 的测试设计章节会把 QAS 映射到具体 unit / integration / simulation 测试用例，并写入「嵌入式风险覆盖矩阵」
- `edf-tdd-implementation` 把 Response Measure 转为 RED 步的判定（例如 latency 直方图、size 工具输出、leak detector 报告）
- `edf-test-checker` 在 `TC4 Embedded Risk Coverage` 维度反向核对：每条 NFR 是否被 `embedded-risk` 用例覆盖
- `edf-code-review` 在 `CR4 Memory & Resource Lifecycle` / `CR5 Concurrency & Real-time` / `CR6 Error Handling & Defensive Design` 维度反向核对实现是否符合 QAS Response Measure
...
送 `edf-spec-review` 前，每条核心 NFR 至少满足：
