---
name: edf-specify
description: Use when the team has accepted an SR / AR / DTS / change request as input and needs it clarified into a reviewable spec. Covers two sub-streams: (a) requirement-analysis sub-stream — clarify subsystem-level SR + optional component implementation design; does NOT proceed to AR-level implementation; (b) implementation sub-stream — clarify AR (or DTS that needs an AR-level spec) before edf-ar-design / edf-tdd-implementation. Not for product discovery (回需求负责人), not for designing the AR implementation (→ edf-ar-design), not for component implementation design authoring (→ edf-component-design), not for hotfix reproduction / root cause (→ edf-problem-fix).
---

# edf 需求规格澄清（覆盖 SR-分析 与 AR-实现 两条子街区）

把团队已经接受的需求输入澄清成可被 `edf-spec-review` 评审的需求规格对象。本 skill 同时服务两个子街区：

1. **需求分析子街区（SR）**：澄清子系统级需求 / 修订；下一步可选地进入 `edf-component-design` 修订组件实现设计；分析阶段以 **analysis closeout** 结束，**不**进入 `edf-ar-design` / TDD / completion-gate。SR 拆出的候选 AR 由需求负责人按 closeout 中的 AR Breakdown Candidates **新建** AR work item，后者重新走实现子街区。
2. **实现子街区（AR / DTS / CHANGE）**：澄清单个 AR（或需要 AR-级规格的 DTS）；下一步进入 `edf-ar-design` 或 `edf-component-design`（component-impact）。
...
- `edf-spec-review` 返回 `需修改` / `阻塞`，需按 findings 修订规格
- 用户说"先把这个 SR / AR 的需求理清楚"

不适用 → 改用：

- 仍在判断该需求是否值得做 → 不属于 df，回需求负责人
- 已有可设计规格，要写 AR 实现设计 → `edf-ar-design`
- 已有可设计规格，要写组件实现设计 → `edf-component-design`
- 紧急缺陷的复现 / 根因 → `edf-problem-fix`
- 阶段不清 / 证据冲突 → `edf-workflow-router`

## Hard Gates

- 需求规格通过 `edf-spec-review` 之前，不得进入 `edf-component-design`、`edf-ar-design`
- SR work item 不得进入 `edf-ar-design` / `edf-tdd-implementation` / `edf-test-checker` / `edf-code-review` / `edf-completion-gate` / `edf-problem-fix`；SR 拆出的候选 AR 必须新建 AR work item
...
- 未经 `using-edf-workflow` / `edf-workflow-router` 入口判断 → 先回 router
...
- **MoSCoW Priority（DSDM, 1994）或团队等价**: row 级 Priority；多条 Must 冲突时回需求负责人；edf 不维护跨工作项 backlog
...
- `DTS`（需要 AR-级规格）→ 实现子街区；通常已先经 `edf-problem-fix` 产出 reproduction / root-cause / fix-design

工件冲突、不确定 work item 类型，或用户描述介于 SR / AR 之间 → 回 `edf-workflow-router`。

### 2. 初始化或对齐 work item 目录

按 Artifact Layout（详见 `docs/edf-shared-conventions.md`），新 work item 用 `templates/edf-work-item-readme-template.md` / `templates/edf-progress-template.md` / `templates/edf-traceability-template.md` 创建骨架；已存在则核对 `progress.md` canonical 字段完整性。
...
每轮结束前总结已锁定与待确认；只剩 1-2 个阻塞事实时合并问。若需要 ≥3 轮且全部依赖业务判断 → 阻塞，回需求负责人；edf 不替业务方拍板。
...
- **SR**：
  ...
  - **AR Breakdown Candidates**：候选 AR 拆分清单（每条候选含范围 / 所属组件 / 优先级 / 上抛对象；本字段在 spec-review 后可能继续修订，最终在 `edf-finalize` analysis closeout 时定稿）
  - **Component Design Impact**：本 SR 是否需要 `edf-component-design`；若需要，要修订哪些章节
- **AR / DTS / CHANGE**：
...
在 `features/<id>/progress.md` 写入 canonical 字段：`Current Stage = edf-specify`、`Workflow Profile = requirement-analysis`（SR）/ router 分配的实现 profile（AR/DTS/CHANGE）、`Pending Reviews And Gates = spec-review`、`Next Action Or Recommended Skill = edf-spec-review`、`Last Updated`。不允许自由文本下一步。
...
按 `edf-workflow-router/references/reviewer-dispatch-protocol.md`，由父会话派发独立 reviewer subagent 执行 `edf-spec-review`（不内联）。同时更新 feature `README.md` 中 Process Artifacts 表的 Requirement 行。自检未过 → 不伪造 handoff，明确写出仍缺什么。
...
- `features/<Work Item Id>-<slug>/progress.md` 已同步：
  - `Current Stage` = `edf-specify`
  - `Workflow Profile` = `requirement-analysis`（SR）/ router 分配的实现 profile（AR / DTS / CHANGE）
  - `Pending Reviews And Gates` 含 `spec-review`
  - `Next Action Or Recommended Skill` = `edf-spec-review`
- `features/<Work Item Id>-<slug>/README.md` 中 Requirement 行更新

handoff 摘要（按 edf-shared-conventions 字段）：`work_item_id`、`owning_component` / `owning_subsystem`、`workflow_profile`、`artifact_paths`、`traceability_links`、`blockers`（如有 USER-INPUT 阻塞项）、`next_action_or_recommended_skill = edf-spec-review`。
...
| 误把组件设计修订写进 requirement.md | 仅在 Component Impact Assessment 标注，由 router 决定是否进入 `edf-component-design` |
...
- [ ] progress.md 已按 canonical schema 同步，含 `Workflow Profile`，下一步为 `edf-spec-review`
...
| `templates/edf-work-item-readme-template.md` | work item README 模板 |
| `templates/edf-progress-template.md` | progress.md 模板 |
| `templates/edf-traceability-template.md` | traceability.md 模板 |
| `docs/edf-shared-conventions.md` | 工件路径、canonical 字段、handoff 字段、requirement.md 必含项 |
| `edf-workflow-router/references/reviewer-dispatch-protocol.md` | reviewer 派发协议 + 定向回修协议（USER-INPUT / LLM-FIXABLE / TEAM-EXPERT 处理顺序、interactive vs auto、单次回合最小问询、反复循环阻断） |
