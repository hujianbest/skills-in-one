# edf AR Test Design Section Contract

> 配套 `edf-ar-design/SKILL.md`。规定 AR 实现设计中**测试设计章节**的最小字段、嵌入式风险覆盖矩阵和与 requirement.md 的双向锚点。

edf 不维护独立 `test-design.md`；测试设计是 AR 实现设计的章节（edf 硬约定）。
...
测试设计章节应说明 TDD 期间需要保留的证据，以保证后续 `edf-test-checker` 可独立审查：
