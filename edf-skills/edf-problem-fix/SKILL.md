---
name: edf-problem-fix
description: Use when a DTS / 紧急缺陷 / 已上线问题 needs reproduction, root-cause analysis, and a minimum-safe fix boundary before any code change, when edf-workflow-router has selected the hotfix profile, or when an AR-route encountered a regression that should be analyzed as a problem before further implementation. Not for writing the fix code (→ edf-tdd-implementation), not for AR design (→ edf-ar-design), not for component design changes (→ edf-component-design), not for stage / route confusion (→ edf-workflow-router).
---
...
本 skill **不**写生产代码（那是 `edf-tdd-implementation` 的职责），**不**修改组件实现设计（必要时回 `edf-component-design`），**不**替开发负责人决定优先级，**不**绕过 review / gate。紧急 ≠ 跳过。
...
- `edf-workflow-router` 已选择 `hotfix` profile
- 用户说"先把这个问题分析清楚再改"

不适用 → 改用：

- 写修复代码 → `edf-tdd-implementation`
- 需求变更 / 范围调整 → 回 `edf-specify`（或 router）
- 修复需要 AR 实现设计 → `edf-ar-design`
- 修复触及组件边界 → `edf-component-design`
- 阶段不清 / 证据冲突 → `edf-workflow-router`

## Hard Gates

- 必须有复现路径（或显式无法复现说明）才能 handoff 给 `edf-tdd-implementation`
- 必须确认根因 + 最小安全修复边界
- 不得把 hotfix 当作跳过 `edf-test-checker` / `edf-code-review` / `edf-completion-gate` 的理由
- 修复边界扩散（跨多模块 / 改公共接口 / 改用户可见行为 / 改数据契约）→ `interactive` 模式下必须先确认边界，`auto` 模式下回 router
- 不替模块架构师决定接口 / 数据契约改动
- 修复若实质上是「新增能力」或「需求变更」→ 不走 hotfix，回 `edf-specify` 或 router
- 不写生产代码
...
- Object Transformation: 把缺陷输入转成可被 `edf-tdd-implementation` 消费的最小修复包
...
如果缺陷描述指向「需求变更」/「新增能力」（既有 spec / 设计未要求当前行为）→ 不走 hotfix，阻塞回 `edf-specify` 或 `edf-workflow-router`。

### 2. 初始化或对齐 work item 目录

按 Artifact Layout（详见 `docs/edf-shared-conventions.md`），新 DTS 用 `templates/edf-work-item-readme-template.md` / `templates/edf-progress-template.md` 创建 `features/DTS<id>-<slug>/README.md` 与 `progress.md`；已存在则核对 canonical 字段。缺 DTS ID / 所属组件 → 阻塞。
...
- `auto`：边界仍清晰且证据充分时继续；否则 `reroute_via_router=true` 回 `edf-workflow-router`
...
| 条件 | 回流节点 |
|---|---|
| 复现 + 根因 + 修复边界清晰、不触组件 / 不需要正式 AR 设计 | `edf-tdd-implementation` |
| 修复需要正式 AR 实现设计 | `edf-ar-design` |
| 修复触及组件边界 | `edf-component-design`（先升 component-impact） |
| 实际是需求变更 | `edf-specify` |
| 证据不足以确认根因 | `edf-workflow-router`（`reroute_via_router=true`） |
...
- `features/DTS<id>-<slug>/progress.md`：`Current Stage = edf-problem-fix`、`Pending Reviews And Gates = test-check, code-review, completion-gate`、`Next Action Or Recommended Skill = <步骤 6 回流节点>`
...
- DTS 触及组件边界仍指向 `edf-tdd-implementation`
...
| DTS 实际是新增能力 | 阻塞，回 `edf-specify` 或 router |
...
| `templates/edf-work-item-readme-template.md` | DTS work item README 模板 |
| `templates/edf-progress-template.md` | progress.md 模板 |
| `docs/edf-shared-conventions.md` | hotfix profile 路径与 canonical 字段 |
| `edf-workflow-router/references/profile-and-route-map.md` | hotfix route 触发条件 |
