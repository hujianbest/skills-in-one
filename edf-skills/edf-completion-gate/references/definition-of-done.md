# edf Definition of Done

> 配套 `edf-completion-gate/SKILL.md`。规定 edf 单 AR / DTS work item 及其 Current Active Task 在不同 profile 下的完成定义。
...
- 模块架构师 sign-off 已记录
- `docs/component-design.md` 的同步已规划，由 `edf-finalize` 完成同步；可选子资产 `docs/interfaces.md` / `docs/dependencies.md` / `docs/runtime-behavior.md` 仅在项目已启用且本次触发变化时纳入同步规划，未启用的把变化合并进 `docs/component-design.md` 对应章节
...
| 条件 | conclusion | 下一步 |
|---|---|---|
| Task-Level DoD 满足且存在唯一 next-ready task，无 critical-open，本轮验证全绿 | `通过` | `edf-tdd-implementation`（锁定 next-ready task） |
| 通用 DoD + Profile 加项均满足，所有 tasks 均 done，无 critical-open，本轮验证全绿 | `通过` | `edf-finalize` |
| 任一项缺失但可在 1-2 轮回修内补齐 | `需修改` | `edf-tdd-implementation` |
| 工具链 / 环境问题阻塞验证 | `阻塞` | `edf-completion-gate`（恢复后重审） |
| profile / route 冲突 / 跨组件协调缺失 / 实质修改组件边界 | `阻塞`（workflow） + `reroute_via_router=true` | `edf-workflow-router` |
