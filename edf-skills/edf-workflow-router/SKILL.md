---
name: edf-workflow-router
description: Use when the user says "继续/推进" and the canonical edf node must be decided from artifact evidence, when a review/gate just finished and orchestration must resume, when route/stage/profile is unclear or artifact evidence conflicts, when deciding whether to enter component-impact / hotfix profile, or when dispatching reviewer subagents for spec/component-design/ar-design/test-check/code-review. Not for new-session family discovery (→ using-edf-workflow), not for authoring/reviewing/implementing inside a leaf node (→ corresponding edf-* skill).
---

# Embedded Dev Flow Workflow Router

Embedded Dev Flow workflow family 的 **runtime authority**。基于工件证据决定：Workflow Profile、Execution Mode、canonical `edf-*` 节点、是否进入 component-impact 或 hotfix 支线、review subagent 派发，以及 review / gate 后的恢复编排。

`using-edf-workflow` 负责 public entry 与意图分流；本 skill 负责 runtime routing 与恢复。

edf 默认以单 AR / 单 DTS 为 work item 边界；实现子街区在 AR 设计通过后维护 work item 内部的 `tasks.md` / `task-board.md` 执行索引。本 skill 不替模块架构师、开发负责人、开发人员拍板任何专业判断；只负责把工件证据转化为唯一下一步。
...
- 需判断是否进入 `edf-component-design`（SR-analysis 可选 / AR component-impact 触发）或 `edf-problem-fix`（hotfix）
...
- 新会话 family discovery → `using-edf-workflow`
- 节点内部 authoring / review / 实现 → 对应 `edf-*` leaf skill
...
- `requirement-analysis` profile 下不得路由到 `edf-ar-design` / `edf-ar-design-review` / `edf-tdd-implementation` / `edf-test-checker` / `edf-code-review` / `edf-completion-gate` / `edf-problem-fix`
- 缺组件实现设计但本次修改影响组件边界 → 必须升级到 `component-impact` profile（仅适用于 AR 工作项），路由到 `edf-component-design`
- AR 实现设计未含测试设计章节 → 不得路由到 `edf-tdd-implementation`，必须回 `edf-ar-design`
- AR 设计通过后 tasks 未产出 / tasks-review 未通过 → 不得路由到 `edf-tdd-implementation`，必须回 `edf-tasks` / `edf-tasks-review`
- task-board 无法唯一判断 `Current Active Task` / next-ready task → 标 `reroute_via_router=true`
- TDD 完成后未经 `edf-test-checker` 审查 → 不得路由到 `edf-code-review`
...
- Invariants: profile / execution mode 一旦决定，不允许 leaf 节点自改；canonical 节点名严格使用 `edf-*` 前缀
...
如果是 public entry / family discovery → 回 `using-edf-workflow`。否则（恢复编排、profile 判断、消费 review/gate 结论、evidence conflict、切支线）继续。
...
| 信号 | 路由 |
|---|---|
| DTS / 紧急缺陷 / 已上线问题修复 | `edf-problem-fix`，profile = `hotfix` |
| 新增组件 / 修改 SOA 接口 / 修改组件职责 / 修改组件依赖 / 组件设计缺失或过期 | profile 升级到 `component-impact`，下一步 `edf-component-design` |
...
特别注意：`requirement-analysis` profile 下，leaf 返回 `edf-ar-design` / `edf-tdd-implementation` / `edf-test-checker` / `edf-code-review` / `edf-completion-gate` / `edf-problem-fix` 一律视为非法 → `reroute_via_router=true`，由真人决定是否新建 AR work item。
...
|---|---|---|---|
| `edf-specify` | `requirement-analysis`（SR） | `edf-spec-review` | 回需求负责人 / `edf-workflow-router` |
| `edf-spec-review` | `requirement-analysis` | `edf-component-design`（本 SR 触发组件设计修订）/ `edf-finalize`（仅澄清，无组件设计修订） | `edf-specify` |
| `edf-component-design` | `requirement-analysis` | `edf-component-design-review` | 继续修订 |
| `edf-component-design-review` | `requirement-analysis` | `edf-finalize`（analysis closeout） | `edf-component-design` |
| `edf-finalize` | `requirement-analysis` | workflow closed（analysis closeout） | 回 router |
| `edf-specify` | 实现 profile | `edf-spec-review` | 回需求负责人 / `edf-workflow-router` |
| `edf-spec-review` | 实现 profile | `edf-component-design`（component-impact）/ `edf-ar-design`（standard / lightweight） | `edf-specify` |
| `edf-component-design` | `component-impact` | `edf-component-design-review` | 继续修订 |
| `edf-component-design-review` | `component-impact` | `edf-ar-design` | `edf-component-design` |
| `edf-ar-design` | 实现 profile | `edf-ar-design-review` | 继续修订 |
| `edf-ar-design-review` | 实现 profile | `edf-tasks` | `edf-ar-design` |
| `edf-tasks` | 实现 profile | `edf-tasks-review` | `edf-tasks` / `edf-ar-design` |
| `edf-tasks-review` | 实现 profile | `edf-tdd-implementation`（锁定 Current Active Task） | `edf-tasks` |
| `edf-tdd-implementation` | 实现 profile | `edf-test-checker` | 继续实现 |
| `edf-test-checker` | 实现 profile | `edf-code-review` | `edf-tdd-implementation` |
| `edf-code-review` | 实现 profile | `edf-completion-gate` | `edf-tdd-implementation` |
| `edf-completion-gate` | 实现 profile | `edf-tdd-implementation`（有唯一 next-ready task）/ `edf-finalize`（无剩余 task） | 缺什么回什么 |
| `edf-finalize` | 实现 profile | workflow closed | 回 router |
| `edf-problem-fix` | `hotfix` | `edf-ar-design` 或 `edf-tdd-implementation` | 继续 hotfix 分析 |
...
- `需修改` / `阻塞`（内容） → 回授权节点（如 `edf-tdd-implementation` / `edf-ar-design`）
- `阻塞`（workflow） → `reroute_via_router=true`，停下并写明阻塞原因

completion-gate 通过后先读取 `Task Board Path`。若存在唯一 `next-ready task`，更新 `Current Active Task` 并路由到 `edf-tdd-implementation`；若不存在剩余 ready / pending task，才路由到 `edf-finalize`；若候选不唯一或状态冲突，回 `edf-workflow-router` hard stop。
...
- `Workflow Profile`
- `Execution Mode`
- `Target Skill`（唯一 canonical `edf-*` 节点）
- `Why`（1-2 条决定性证据）
- `reroute_via_router`：`false`（已唯一映射）或 `true`（无法唯一映射，等待父会话）

evidence 充足时使用紧凑格式；不回放未命中分支，不复述 authority 说明。

runtime canonical 字段统一：`edf-workflow-router`、`reroute_via_router`，不出现自由文本下一步。

## Red Flags

- 没经过 router 就跨节点切换
- 因命令名 / 用户点名跳过 route / profile 判断
- 把 `using-edf-workflow` 写进 runtime handoff
- 在 route 阶段做大范围代码探索
- 忽略证据冲突沿用旧印象推进
...
| 错误 | 修复 |
|---|---|
| TDD 完成后直接路由到 `edf-code-review` | 必须先派发 `edf-test-checker` |
| 看到 AR 设计修改了组件接口，仍走 standard | 升级到 component-impact，先 `edf-component-design` |
...
- [ ] 字段名严格使用 `edf-workflow-router` 与 `reroute_via_router`

## Supporting References

| 文件 | 用途 |
|---|---|
| `docs/edf-shared-conventions.md` | profile 与节点合法集合、handoff 字段、路径约定 |
