# edf Profile And Route Map

> 配套 `edf-workflow-router/SKILL.md`。展开 edf 各 Workflow Profile 的合法节点集合、主链与支线，以及 hard stops 的具体表现。
...
using-edf-workflow
  -> edf-workflow-router
  -> edf-specify                       (SR 子系统级需求澄清)
  -> edf-spec-review
  -> (可选) edf-component-design       (仅当 SR 触发组件实现设计修订)
  -> (可选) edf-component-design-review
  -> edf-finalize                      (analysis closeout)
```

合法节点集合：

```text
{ edf-specify, edf-spec-review,
  edf-component-design, edf-component-design-review,
  edf-finalize }
```

非法节点（出现 → router hard stop，标 `reroute_via_router=true`）：

- `edf-ar-design` / `edf-ar-design-review` / `edf-tdd-implementation` / `edf-test-checker` / `edf-code-review` / `edf-completion-gate` / `edf-problem-fix`
...
完成方式：`edf-finalize` 写 **analysis closeout**，含：
...
using-edf-workflow
  ...
  -> edf-code-review
  -> edf-completion-gate
  -> (next-ready task ? edf-tdd-implementation : edf-finalize)
```

合法节点集合：

```text
{ edf-specify, edf-spec-review, edf-ar-design, edf-ar-design-review,
  edf-tasks, edf-tasks-review,
  edf-tdd-implementation, edf-test-checker, edf-code-review,
  edf-completion-gate, edf-finalize }
```
...
using-edf-workflow
  ...
  -> edf-specify
  -> edf-spec-review
  -> edf-component-design
  -> edf-component-design-review
  -> edf-ar-design
  -> edf-ar-design-review
  -> edf-tasks
  -> edf-tasks-review
...
注意：组件实现设计是 AR 实现设计的输入。AR 实现设计**不得**临时改写组件架构；必须先经过 `edf-component-design`。
...
using-edf-workflow
  -> edf-workflow-router
  -> edf-problem-fix
  -> (可选) edf-ar-design -> edf-ar-design-review
  -> (可选) edf-tasks -> edf-tasks-review
  -> edf-tdd-implementation
  -> edf-test-checker
  -> edf-code-review
  -> edf-completion-gate
...
`edf-problem-fix` 至少完成：复现路径或无法复现说明、根因、最小安全修复边界、是否需要补 AR 实现设计或组件实现设计、明确回流节点。
...
| `component-impact` | 发现是 DTS | 仍 `component-impact` 兼 hotfix 性质，启动 `edf-problem-fix` 子线 |
...
1. 需求输入不清且涉及方向 / 范围 / 验收 → 停在 `edf-specify`，回需求负责人
2. IR / SR / AR 追溯关系冲突 → 阻塞，回需求负责人
3. AR 不属于唯一组件（实现子街区） → 阻塞
4. SR 缺所属子系统（需求分析子街区） → 阻塞
5. SR-flow 试图进入 `edf-ar-design` / `edf-tdd-implementation` / 任何实现节点 → router hard stop，提示新建 AR work item
6. SR analysis closeout 缺 AR Breakdown Candidates 且未声明「无可拆分 AR」 → 阻塞
7. 缺组件实现设计但当前修改影响组件边界（实现子街区） → 进 `edf-component-design` 并升级 component-impact
8. AR 实现设计未含测试设计章节 → 回 `edf-ar-design`
9. AR 设计通过后 tasks 未产出 / tasks-review 未通过 → 不得进 `edf-tdd-implementation`
10. task-board 中 `Current Active Task` 不唯一、多个 `in_progress` 或依赖状态冲突 → 回 router
11. TDD 完成后测试用例未经 `edf-test-checker` → 不得进 `edf-code-review`
...
| `edf-specify` | `edf-spec-review` |
| `edf-component-design` | `edf-component-design-review` |
| `edf-ar-design` | `edf-ar-design-review` |
| `edf-tasks` | `edf-tasks-review` |
| `edf-tdd-implementation` | `edf-test-checker` |
| `edf-test-checker`（通过） | `edf-code-review` |
