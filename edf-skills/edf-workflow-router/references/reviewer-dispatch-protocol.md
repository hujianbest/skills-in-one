# edf Reviewer Dispatch Protocol

> 配套 `edf-workflow-router/SKILL.md`。规定 edf 中 review 节点如何派发独立 reviewer subagent，以及 reviewer 返回结果的契约。

## 角色边界

edf 的 review 节点（spec-review / component-design-review / ar-design-review / tasks-review / test-checker / code-review）必须由**独立 reviewer 角色或 subagent** 执行，理由：
...
- `target_skill`：`edf-spec-review` / `edf-component-design-review` / `edf-ar-design-review` / `edf-tasks-review` / `edf-test-checker` / `edf-code-review`
...
- `supporting_context`：上游工件路径列表（按目标 skill 与 work item 类型）：
  ...
  - tasks-review（**仅** AR / DTS / CHANGE）：`features/<id>/tasks.md` + `features/<id>/task-board.md` + `features/<id>/ar-design-draft.md` + `features/<id>/requirement.md`
...
- `expected_record_path`：默认见 `docs/edf-shared-conventions.md`，项目覆写优先
...
- 若问题本质属于 stage / route / profile 冲突，必须 `reroute_via_router=true` 且 `next_action_or_recommended_skill=edf-workflow-router`
...
|---|---|---|---|---|
| `edf-spec-review` | `edf-component-design`（SR 触发组件设计修订）/ `edf-finalize`（仅澄清，无组件设计修订；写 analysis closeout） | `edf-specify` | `edf-specify` | `edf-workflow-router` |
| `edf-component-design-review` | `edf-finalize`（写 analysis closeout） | `edf-component-design` | `edf-component-design` | `edf-workflow-router` |

SR-flow 中 `edf-ar-design-review` / `edf-tasks-review` / `edf-test-checker` / `edf-code-review` 不应被派发；如果发生，`edf-workflow-router` 应在派发前拦截，或 reviewer 返回 `阻塞`(workflow) + `reroute_via_router=true`。

### 实现子街区（profile = `standard` / `component-impact` / `hotfix` / `lightweight`）

| Reviewer | 通过 | 需修改 | 阻塞（内容） | 阻塞（workflow） |
|---|---|---|---|---|
| `edf-spec-review` | `edf-component-design`（component-impact）/ `edf-ar-design`（其余） | `edf-specify` | `edf-specify` | `edf-workflow-router` |
| `edf-component-design-review` | `edf-ar-design` | `edf-component-design` | `edf-component-design` | `edf-workflow-router` |
| `edf-ar-design-review` | `edf-tasks` | `edf-ar-design` | `edf-ar-design` | `edf-workflow-router` |
| `edf-tasks-review` | `edf-tdd-implementation` | `edf-tasks` | `edf-tasks` / `edf-ar-design` | `edf-workflow-router` |
| `edf-test-checker` | `edf-code-review` | `edf-tdd-implementation` | `edf-tdd-implementation` | `edf-workflow-router` |
| `edf-code-review` | `edf-completion-gate` | `edf-tdd-implementation` | `edf-tdd-implementation` | `edf-workflow-router` |
...
reviewer 给出 `需修改` / `阻塞`（内容）verdict 后，回到上游 authoring 节点（`edf-specify` / `edf-component-design` / `edf-ar-design` / `edf-tdd-implementation`）做**定向回修**——不重新发散整份草稿。本节适用所有 edf review 节点。
...
- 一次回合中，向用户提的 USER-INPUT 问题数量上限：**5**（更多 → 说明本轮 reviewer 已暴露规格根本缺口，应回 `edf-specify` 重做澄清）
...
- 如果反复循环的根因是 stage / route / profile 冲突 → 不在 authoring 节点硬修，回 `edf-workflow-router` 重新分流
