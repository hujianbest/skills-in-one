---
name: edf-spec-review
description: Use when a requirement.md draft from edf-specify is ready for an independent verdict (covering both the requirement-analysis sub-stream for SR work items and the implementation sub-stream for AR / DTS / CHANGE work items), when a reviewer subagent is dispatched to evaluate the spec for clarity / traceability / designability, or when spec-review needs to be re-run after the author revised in response to earlier findings. Not for writing or revising the spec itself (→ edf-specify), not for component or AR design review (→ edf-component-design-review / edf-ar-design-review), not for stage / route confusion (→ edf-workflow-router).
---

# edf 需求规格评审（覆盖 SR-分析 与 AR-实现 两条子街区）

独立评审 `features/<id>/requirement.md`：

- **SR work item**（profile = `requirement-analysis`）→ 判断它是否可作为 `edf-component-design`（仅当 SR 触发组件设计修订）或直接 `edf-finalize`（analysis closeout）的稳定输入；同时把 AR Breakdown Candidates 推到可被需求负责人决策的程度
- **AR / DTS / CHANGE work item**（实现 profile）→ 判断它是否可作为 `edf-component-design`（component-impact）或 `edf-ar-design`（standard / lightweight）的稳定输入
...
- `edf-specify` 已产出 requirement.md 草稿（SR 或 AR / DTS / CHANGE），需正式 verdict
- reviewer subagent 被派发执行 spec 评审
- 用户明确要求「review 这份规格 / 评审需求」

不适用 → 改用：

- 缺规格或仅需继续写 → `edf-specify`
- 阶段不清 / 证据冲突 → `edf-workflow-router`
- 已有批准规格、需要做组件 / AR 设计评审 → `edf-component-design-review` / `edf-ar-design-review`

## Hard Gates

- 规格通过本 review 之前，不得进入 `edf-component-design` 或 `edf-ar-design`
- reviewer 不修改 requirement.md
- reviewer 不替需求负责人 / 模块架构师补业务事实、优先级、阈值
- reviewer 不返回多个候选下一步
- 工件不足以判定 stage / route → `reroute_via_router=true`，回 `edf-workflow-router`
...
- Object Invariants: verdict 必为 `通过` / `需修改` / `阻塞` 之一，下一步必为 canonical edf-* 节点名
...
- 缺规格 / 缺 traceability → 写最小 blocked record，下一步 `edf-specify`
- route / stage / profile 冲突 → 写最小 blocked record，`reroute_via_router=true`，下一步 `edf-workflow-router`
- 否则进入步骤 2
...
| SR | S8-SR Component Design Impact | 若本 SR 触发 `edf-component-design`，已显式列出受影响组件设计章节与修订方向 |
...
|---|---|---|---|
| 范围清晰、Affected Components / AR Breakdown Candidates 章节充分、Component Design Impact 显式判断为「需修订组件设计」、无阻塞 USER-INPUT | `通过` | `edf-component-design` | `false` |
| 范围清晰、Affected Components / AR Breakdown Candidates 章节充分、Component Design Impact 显式判断为「无需修订」、无阻塞 USER-INPUT | `通过` | `edf-finalize`（analysis closeout） | `false` |
| 有用但不完整，findings 可 1-2 轮定向修订 | `需修改` | `edf-specify` | `false` |
| 子系统范围 / 候选 AR 拆分严重不清，findings 无法定向回修 | `阻塞`（内容） | `edf-specify` | `false` |
| route / stage / profile / 上游证据冲突；或 SR 工件试图映射到实现节点 | `阻塞`（workflow） | `edf-workflow-router` | `true` |

**AR / DTS / CHANGE work item（实现 profile）**：

| 条件 | conclusion | next_action_or_recommended_skill | reroute_via_router |
|---|---|---|---|
| 范围清晰、核心 rows 含 Acceptance、Component Impact 已判断、无阻塞 USER-INPUT、足以喂下一节点 | `通过` | `edf-component-design`（Component Impact ≠ none）/ `edf-ar-design`（其余） | `false` |
| 有用但不完整，findings 可 1-2 轮定向修订 | `需修改` | `edf-specify` | `false` |
| 范围 / 验收 / 组件归属严重不清，findings 无法定向回修 | `阻塞`（内容） | `edf-specify` | `false` |
| route / stage / profile / 上游证据冲突 | `阻塞`（workflow） | `edf-workflow-router` | `true` |

### 5. 写 review 记录并回传

按 `templates/edf-review-record-template.md` 写 `features/<id>/reviews/spec-review.md`，并按 `edf-workflow-router/references/reviewer-dispatch-protocol.md` 回传结构化摘要（`record_path` / `conclusion` / `key_findings` / `finding_breakdown` / `next_action_or_recommended_skill` / `needs_human_confirmation` / `reroute_via_router`）。USER-INPUT 阻塞项必须显式列出，让父会话上抛需求负责人；LLM-FIXABLE 不要抛给用户。
...
- 结构化 reviewer 返回摘要含：
  ...
  - `next_action_or_recommended_skill`：唯一 canonical edf-* 节点
...
| `docs/edf-shared-conventions.md` | handoff 字段、路径约定 |
