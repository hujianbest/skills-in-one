# edf Code Review Rubric

> 配套 `edf-code-review/SKILL.md`。展开 8 维度评分细则与 rule IDs。
...
| 测试代码角度 | 测试命名、fixture / mock 使用、边界 / 异常覆盖与 `edf-test-checker` 结论一致；发现明显测试问题时回 `edf-test-checker` / `edf-tdd-implementation` |
...
- `LLM-FIXABLE`：代码结构 / 错误处理 / 命名 / 边界 / 防御性检查 / Refactor Note 字段补全 / in-task 范围 smell → 由 `edf-tdd-implementation` 回修
