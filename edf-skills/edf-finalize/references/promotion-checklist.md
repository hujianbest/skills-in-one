# edf Long-Term Asset Promotion Checklist

> 配套 `edf-finalize/SKILL.md`。规定如何把 `features/<id>/` 的过程产物正确 promote 到组件仓库 `docs/`，并在 closeout pack 中记录同步路径。
...
> 「视情况同步」列中的可选子资产，未启用时统一在 `docs/component-design.md` 中维护对应章节；**不**因为 closeout 触发就自动创建可选子资产文件。是否要在本轮 closeout 中**首次启用**某个可选子资产（例如本次 SR / AR 引入了团队此前未单独维护的运行时行为约定），由模块架构师 / 开发负责人决定，edf 不自动创建。

> SR 拆出的候选 AR **不**由 edf 自动新建 work item。`AR Breakdown Candidates` 是给需求负责人的交付清单；新建 AR work item 由需求负责人按团队优先级决定，由 router 重新分流走实现子街区。
...
2. **补长期文档结构**：保留团队组件设计 / AR 设计模板的全套章节（包括 edf 占位模板未补齐时由开发负责人 / 模块架构师手动补齐的章节）
...
`templates/edf-closeout-template.md` 的 `Long-Term Assets Sync` 表必须显式列出：
