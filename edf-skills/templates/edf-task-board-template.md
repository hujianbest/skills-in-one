...
- 这是 `edf-tasks` 用于 task-to-task 推进的队列投影模板，默认保存为 `features/<Work Item Id>-<slug>/task-board.md`。
...
- 若 task board 与已批准 tasks.md 冲突，或无法唯一判断 next-ready task，停止自动推进并回 `edf-workflow-router`。
...
- Conflict Policy:
  - 若存在多个同等候选，或依赖 / 状态冲突，则回 `edf-workflow-router`
- Ready Semantics:
- Done Semantics:
