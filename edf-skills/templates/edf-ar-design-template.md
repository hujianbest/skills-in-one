...
- 本模板用于 `edf-ar-design` 产出 `features/<id>/ar-design-draft.md`，并在 closeout 后 promote 到 `docs/ar-designs/AR<id>-<slug>.md`。
...
如本 AR 需要重构，必须说明触发原因、范围、受影响模块和验证方式；跨组件边界或跨 >=3 模块结构性重构应回 `edf-workflow-router`。
...
对接口、算法、关键功能进行测试策略和测试场景设计，同时对可测试性需求进行设计。测试设计驱动后续 `edf-tasks` 与 `edf-tdd-implementation`。每个用例必须回指功能点和 requirement row。
