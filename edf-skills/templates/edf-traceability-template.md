...
- 由 `edf-specify` 初始化骨架，后续 `edf-component-design` / `edf-ar-design` / `edf-tasks` / `edf-tdd-implementation` / `edf-test-checker` / `edf-code-review` 各自补充本节点对应行（实现子街区）；SR 由 `edf-specify` / `edf-component-design` 补充，止于 candidate breakdown 列。
