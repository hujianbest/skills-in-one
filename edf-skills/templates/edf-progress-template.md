# edf Work Item Progress

使用说明：

- 这是 `features/<Work Item Id>-<slug>/progress.md` 模板。
- 是 work item 唯一权威 progress 落点，所有 `edf-*` skill 完成节点工作时必须用 canonical 字段同步。
- `Next Action Or Recommended Skill` 只能写 canonical `edf-*` 节点名，**不允许**自由文本，**不允许**写 `using-edf-workflow`。
...
- Current Stage:                           # canonical edf-* 节点
...
- Next Action Or Recommended Skill:        # canonical edf-* 节点
- Notes:
