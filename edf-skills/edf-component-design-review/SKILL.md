---
name: edf-component-design-review
description: Use when edf-component-design has produced a component-design-draft.md ready for an independent verdict (covering both SR-triggered analysis sub-stream and AR-triggered component-impact sub-stream), when a reviewer subagent is dispatched to evaluate component boundaries / SOA interfaces / dependencies / state machines / runtime mechanisms, or when component design needs re-review after revision. Not for writing or revising component design (→ edf-component-design), not for AR-level design review (→ edf-ar-design-review), not for stage / route confusion (→ edf-workflow-router).
---

# edf 组件实现设计评审（覆盖 SR-分析 与 AR-实现 两条子街区）

独立评审 `features/<id>/component-design-draft.md`，判断它是否可由 `edf-finalize` 同步到 `docs/component-design.md`，以及（仅 AR 实现子街区）是否可作为下游 `edf-ar-design` 的稳定输入。
...
- `edf-component-design` 已产出 component-design-draft.md，需正式 verdict
- reviewer subagent 被派发执行组件设计评审
- 用户明确要求「review 这份组件设计」

不适用 → 改用：

- 缺草稿或仅需继续写 → `edf-component-design`
- AR 代码层设计评审 → `edf-ar-design-review`
- 阶段不清 / 证据冲突 → `edf-workflow-router`

## Hard Gates

- AR 子街区中，组件设计通过本 review 之前不得进入 `edf-ar-design`
- SR 子街区中，组件设计通过本 review 之前不得进入 `edf-finalize`（analysis closeout）；通过后 SR 子街区**仍不得**进入 `edf-ar-design`
- reviewer 不修改设计草稿
...
- 缺设计草稿 → blocked-content，下一步 `edf-component-design`
- route / stage / profile 冲突（如未升级 component-impact 却进入本节点） → blocked-workflow，`reroute_via_router=true`，下一步 `edf-workflow-router`
...
|---|---|---|---|---|
| 7 维度均 ≥ 6、无 critical USER-INPUT、模块架构师可被请求 sign-off | `通过` | `edf-finalize`（analysis closeout） | `false` | `true`（等模块架构师 sign-off） |
| findings 可 1-2 轮定向修订 | `需修改` | `edf-component-design` | `false` | `false` |
| 组件边界 / SOA 接口严重不清 / 跨组件协调缺失 | `阻塞`（内容） | `edf-component-design` | `false` | `false` |
| route / stage / profile / 上游证据冲突；或 SR 工件试图映射到 `edf-ar-design` | `阻塞`（workflow） | `edf-workflow-router` | `true` | `false` |

**AR work item（profile = `component-impact`）**：

| 条件 | conclusion | next_action_or_recommended_skill | reroute_via_router | needs_human_confirmation |
|---|---|---|---|---|
| 7 维度均 ≥ 6、无 critical USER-INPUT、模块架构师可被请求 sign-off | `通过` | `edf-ar-design` | `false` | `true`（等模块架构师 sign-off） |
| findings 可 1-2 轮定向修订 | `需修改` | `edf-component-design` | `false` | `false` |
| 组件边界 / SOA 接口严重不清 / 跨组件协调缺失 | `阻塞`（内容） | `edf-component-design` | `false` | `false` |
| route / stage / profile / 上游证据冲突 | `阻塞`（workflow） | `edf-workflow-router` | `true` | `false` |

### 5. 写 review 记录并回传

按 `templates/edf-review-record-template.md` 写 `features/<id>/reviews/component-design-review.md`，并按 `edf-workflow-router/references/reviewer-dispatch-protocol.md` 回传结构化摘要。`通过` 时 `needs_human_confirmation=true`，等模块架构师 sign-off 后由父会话进入 `edf-ar-design`。
...
- `通过` 时 needs_human_confirmation 默认 `true`（等模块架构师 sign-off），由父会话决定何时进入 `edf-ar-design`
