---
name: edf-ar-design-review
description: Use when edf-ar-design has produced an ar-design-draft.md ready for an independent verdict, when a reviewer subagent is dispatched to evaluate the AR-level code design and its embedded test-design section, or when AR design needs re-review after revision. Not for writing or revising AR design (→ edf-ar-design), not for component design review (→ edf-component-design-review), not for stage / route confusion (→ edf-workflow-router).
---

# edf AR 实现设计评审

独立评审 `features/<id>/ar-design-draft.md`，判断它是否可作为 `edf-tasks` 的稳定输入，以及测试设计章节是否足以驱动后续 TDD。
...
- `edf-ar-design` 已产出 ar-design-draft.md，需正式 verdict
- reviewer subagent 被派发执行 AR 设计评审
- 用户明确要求「review 这份 AR 设计 / 评审 AR 实现设计」

不适用 → 改用：

- 缺草稿或仅需继续写 → `edf-ar-design`
- 组件级设计评审 → `edf-component-design-review`
- 阶段不清 / 证据冲突 → `edf-workflow-router`

## Hard Gates

- AR 设计通过本 review 之前，不得进入 `edf-tasks` / `edf-tdd-implementation`
- reviewer 不修改设计草稿、不补写测试用例
- reviewer 不替开发负责人 / 模块架构师拍板
- reviewer 不返回多个候选下一步
- AR 设计若改写组件接口 / 依赖 / 状态机 → 强制 `阻塞`(workflow)，下一步 `edf-workflow-router` 升级 component-impact
...
- 缺 ar-design-draft.md → blocked-content，下一步 `edf-ar-design`
- 缺组件设计且本 AR 触及组件边界 → blocked-workflow，`reroute_via_router=true`，下一步 `edf-workflow-router`
- 否则进入步骤 2

### 2. 检查是否触及组件边界

按 SOA Boundary 检查对照 AR 设计与当前 `docs/component-design.md`。**触及组件接口 / 依赖 / 状态机** → 强制 `阻塞`(workflow)，`reroute_via_router=true`，下一步 `edf-workflow-router`（升级 component-impact）。
...
按 Checklist-Based Review（详见 `references/ar-design-review-rubric.md` Group AD1-AD8 子规则）逐项审查；每条 finding 带 `severity` / `classification` / `rule_id` / `anchor` / 描述 / 建议修复。Test Design Adequacy 维度按 `edf-ar-design/references/test-design-section-contract.md` 校验最小字段。
...
|---|---|---|---|---|
| 8 维度均 ≥ 6、组件边界未被改写、测试设计章节充分、无 critical USER-INPUT | `通过` | `edf-tasks` | `false` | `true`（开发负责人确认进入任务执行索引） |
| findings 可 1-2 轮定向修订 | `需修改` | `edf-ar-design` | `false` | `false` |
| 测试设计章节缺失 / 测试设计被拆独立文件 / 嵌入式风险覆盖矩阵缺失 / 设计严重不清 | `阻塞`（内容） | `edf-ar-design` | `false` | `false` |
| AR 设计修改组件接口 / 依赖 / 状态机 / 上游证据冲突 | `阻塞`（workflow） | `edf-workflow-router` | `true` | `false` |

### 6. 写 review 记录并回传

按 `templates/edf-review-record-template.md` 写 `features/<id>/reviews/ar-design-review.md`，并按 `edf-workflow-router/references/reviewer-dispatch-protocol.md` 回传结构化摘要。`通过` 时 `needs_human_confirmation=true`，等开发负责人确认后由父会话进入 `edf-tasks`。
...
| `edf-ar-design/references/test-design-section-contract.md` | 测试设计章节最小契约 |
