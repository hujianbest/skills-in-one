# devflow Embedded Evidence Checklist

> 配套 `devflow-tdd-implementation/SKILL.md`。规定嵌入式 C / C++ TDD 实现期间需保留的静态 / 动态质量证据。

## 证据落点

```text
features/<id>/evidence/
  test-harness/               # 新增组件 / 新测试目标的测试工程 bootstrap 证据
...
## 测试 harness bootstrap 证据

适用条件：

- 新增组件没有现成 test target / runner / fixture
- 当前 task 首次为某组件接入单元 / 集成 / 仿真测试
- 团队测试框架位置不明确，需要从 `AGENTS.md`、构建脚本、CI、相邻组件测试中探索

必含：

- 上下文探索摘要：读取过的 `AGENTS.md`、测试目录、构建脚本、CI 配置、相邻组件测试
- 采用的测试框架 / mock / fixture 风格及理由
- 新增或修改的 test target / runner / fixture / mock 接入文件
- smoke / empty test 命令、退出码、通过摘要
- 仍未明确的团队约定与 reviewer 关注点

bootstrap 证据只证明测试工程可运行，不证明 AR 行为。runner 不存在、test target 未注册、fixture 未接入、构建系统未识别测试文件等失败，不能作为 `RED-<case-id>`。
