...
devflow 的 review 节点（spec-review / component-design-review / ar-design-review / task queue preflight / test-checker / code-review）必须由**独立 reviewer 角色或 subagent** 执行，理由：
...
- `target_skill`：`devflow-spec-review` / `devflow-component-design-review` / `devflow-ar-design-review` / `devflow-tdd-implementation` task queue preflight / `devflow-test-checker` / `devflow-code-review`
...
- `supporting_context`：上游工件路径列表（按目标 skill 与 work item 类型）：
  ...
  - task queue preflight（**仅** AR / DTS / CHANGE）：`features/<id>/tasks.md` + `features/<id>/task-board.md` + `features/<id>/ar-design-draft.md` + `features/<id>/requirement.md`
...
- `expected_record_path`：默认见 ?? skill ? `SKILL.md` / Local DevFlow Conventions，项目覆写优先
...
SR-flow 中 `devflow-ar-design-review` / `devflow-tdd-implementation` task queue preflight / `devflow-test-checker` / `devflow-code-review` 不应被派发；如果发生，`devflow-router` 应在派发前拦截，或 reviewer 返回 `阻塞`(workflow) + `reroute_via_router=true`。
...
| `devflow-tdd-implementation` task queue preflight | `devflow-tdd-implementation` | `devflow-tdd-implementation` | `devflow-tdd-implementation` / `devflow-ar-design` | `devflow-router` |
