...
- AR 设计通过后 tasks 未产出 / task queue preflight 未通过 → 不得路由到 `devflow-tdd-implementation`，必须回 `devflow-tdd-implementation` / `devflow-tdd-implementation` task queue preflight
...
| `devflow-ar-design-review` | 实现 profile | `devflow-tdd-implementation` | `devflow-ar-design` |
| `devflow-tdd-implementation` | 实现 profile | `devflow-tdd-implementation` task queue preflight | `devflow-tdd-implementation` / `devflow-ar-design` |
| `devflow-tdd-implementation` task queue preflight | 实现 profile | `devflow-tdd-implementation`（锁定 Current Active Task） | `devflow-tdd-implementation` |
| `devflow-tdd-implementation` | 实现 profile | `devflow-test-checker` | 继续实现 |
