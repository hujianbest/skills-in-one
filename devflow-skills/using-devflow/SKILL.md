---
name: using-devflow
description: Use when a new session needs to enter the DevFlow workflow for an AR / DTS / change request, when the user expresses a generic intent like "继续这个 AR" / "开始处理这个缺陷" / "做规格澄清" / "做 AR 设计" / "做 TDD" / "做 review" without confirming the canonical devflow node, or when deciding direct invoke vs route-first for a single devflow-* skill. Not for runtime recovery or profile decisions (→ devflow-router), not when already inside a leaf skill (→ continue current skill), not for product discovery (devflow does not own product discovery — escalate to product owner).
---
...
| 写 / 修任务执行索引（tasks.md / task-board.md） | `devflow-tasks` | 实现 |
| 评审任务执行索引 | `devflow-tasks-review` | 实现 |
| TDD 实现 / 改代码 | `devflow-tdd-implementation` | 实现 |
| TDD 后审查测试用例有效性 | `devflow-test-checker` | 实现 |
...
`devflow-completion-gate` / `devflow-ar-design` / `devflow-tdd-implementation` / `devflow-test-checker` / `devflow-code-review` / `devflow-problem-fix` **不**适用于 SR 工作项；SR 走 `requirement-analysis` profile，仅经过 specify → spec-review → (可选) component-design → finalize。
...
| `/devflow-tasks` | `devflow-tasks` |
| `/devflow-build` / `/devflow-tdd` | `devflow-tdd-implementation` |
| `/devflow-test-check` | `devflow-test-checker` |
...
| 错误 | 修复 |
|---|---|
| 用户说「我要做这个 AR」就直接进入 `devflow-tdd-implementation` | 先确认是否有 AR 实现设计和已批准 tasks；缺设计进 `devflow-ar-design`，缺 tasks 进 `devflow-tasks` |
...
- using-devflow is a public entry skill only; never write it into runtime handoff.
- Direct invoke is allowed only when the target leaf skill and required artifacts are clear.
- Route-first goes to devflow-router when stage, profile, route, or evidence is unclear.

### Canonical Nodes

using-devflow, devflow-router, devflow-specify, devflow-spec-review, devflow-component-design, devflow-component-design-review, devflow-ar-design, devflow-ar-design-review, devflow-tasks, devflow-tasks-review, devflow-tdd-implementation, devflow-test-checker, devflow-code-review, devflow-completion-gate, devflow-finalize, devflow-problem-fix.
