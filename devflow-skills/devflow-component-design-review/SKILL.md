...
description: Use when devflow-component-design has produced a component-design-draft.md ready for an independent verdict (covering both SR-triggered analysis sub-stream and AR-triggered component-impact sub-stream), when a reviewer subagent is dispatched to evaluate component boundaries / SOA interfaces / dependencies / state machines / runtime mechanisms, or when component design needs re-review after revision. Not for writing or revising component design (→ devflow-component-design), not for AR-level design review (→ devflow-ar-design-review), not for stage / route confusion (→ devflow-router).
---
...
按 `references/devflow-review-record-template.md` 写 `features/<id>/reviews/component-design-review.md`，并按 `references/reviewer-dispatch-protocol.md` 回传结构化摘要。`通过` 时 `needs_human_confirmation=true`，等模块架构师 sign-off 后由父会话进入 `devflow-ar-design`。
...
### Component Design Review Record

Write features/<id>/reviews/component-design-review.md unless AGENTS.md overrides it.

### Review Boundary

Review component responsibilities, SOA interfaces, dependencies, state machines, runtime behavior, optional long-term assets, and role ownership. Do not revise the design inside the review skill.
