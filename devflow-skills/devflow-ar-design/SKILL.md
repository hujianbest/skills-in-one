...
- 已有可消费的 AR 设计但尚未形成 tasks → `devflow-tasks`；tasks-review 已通过且 active task 已锁定才进入 `devflow-tdd-implementation`
...
- AR 实现设计 review 通过前，`devflow-tasks` 不得开始；`devflow-tdd-implementation` 只能消费已批准 AR 设计 + 已批准 tasks 的当前 active task
...
进入 handoff 前自检：旧 AR 模板章节齐全；无 `AI提示`、示例业务内容、变量替换说明、`TBD` / `{DATE}` 等残留；动态行为 / 流程 / 类图等图形已替换为真实内容或明确 N/A 理由；功能点可追溯且可被测试设计覆盖；MDC 五场景均已分析；接口描述含参数 / 返回值 / 错误码 / 并发约束；与组件设计一致且未修改组件接口；测试设计章节存在且每个用例回指 requirement row + 功能点；嵌入式风险覆盖矩阵完整；风险与未决问题已分类。任一失败 → 回步骤 4 / 5。自检通过 → 父会话派发独立 reviewer subagent 执行 `devflow-ar-design-review`。
