...
- 这是 `devflow-finalize` 的 closeout 记录模板，覆盖两类 closeout：
  - **Implementation closeout**：AR / DTS / CHANGE，基于 `devflow-completion-gate` 通过
  - **Analysis closeout**：SR，基于 `devflow-spec-review`（不修订组件设计）或 `devflow-component-design-review`（修订组件设计）通过
- **默认保存路径：`features/<Work Item Id>-<slug>/closeout.md`**
- devflow 单 work item 一次 finalize；实现子街区只有在 task-board 中所有任务均完成或取消后才 closeout。SR 拆出的候选 AR 由需求负责人**新建** AR work item，**不**由 devflow 自动创建。
...
由 SR `devflow-specify` 起草、`devflow-spec-review` 修订、`devflow-finalize` analysis closeout 时定稿。每条候选含：
