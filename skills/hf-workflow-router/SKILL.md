...
每次进入 runtime routing 之前，先加载并按 4 条准则做一次最小自检：Think Before Coding / Simplicity First / Surgical Changes / Goal-Driven Execution。

- 自检结果**不**改变 router 的 authoritative 输出（profile / mode / canonical 节点），仅约束 router 在描述判断、列 trade-off 时的行为基线。
- **不**把 `hf-coding-principles` 写进 `Current Stage`、`Next Action Or Recommended Skill` 或迁移表。
- 自检完成后进入步骤 1。
...
| 文件 | 用途 |
|------|------|
| `../hf-coding-principles/SKILL.md` | always-on 行为前置层，runtime routing 前先加载 |
