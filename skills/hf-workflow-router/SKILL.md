...
只读路由所需的最少内容：`AGENTS.md` 路径映射、用户请求、工件状态、`docs/index.md` 中标注的当前 active feature、`features/<active>/progress.md`、`features/<active>/reviews/` 与 `features/<active>/verification/` 等 review/verification artifacts。不先做大范围代码探索。证据冲突时按未批准处理、选择更上游节点、必要时升级 profile。
