...
### Group CR1 - Correctness（正确性）（正确性）（正确性）

- `CR1.1` 实现完整覆盖 AR 行为
- `CR1.2` 边界条件处理正确
- `CR1.3` 死代码 / 不可达分支已清理或解释

### Group CR2 - Design Conformance（设计一致性）（设计一致性）（设计一致性）

- `CR2.1` 实现遵循 ar-design-draft.md
- `CR2.2` 与 component-design.md 一致
- `CR2.3` 偏离设计的部分有显式理由 + 追溯锚点

### Group CR3 - SOA Boundary（SOA 边界）（SOA 边界）（SOA 边界）
...
### Group CR4 - Memory & Resource（内存与资源）（内存与资源）（内存与资源）
...
### Group CR5 - Concurrency & Real-time（并发与实时性）（并发与实时性）（并发与实时性）
...
### Group CR6 - Error Handling（错误处理）（错误处理）（错误处理）
...
### Group CR7 - Coding Standard & Static Analysis（编码规范与静态分析）（编码规范与静态分析）（编码规范与静态分析）
...
### Group CR8 - Refactor Note & Health（重构记录与架构健康）（重构记录与架构健康）（重构记录与架构健康）
...
| 测试代码角度 | 测试命名、fixture / mock 使用、边界 / 异常覆盖与 `devflow-test-checker` 结论一致；发现明显测试问题时回 `devflow-test-checker` / `devflow-tdd-implementation` |
