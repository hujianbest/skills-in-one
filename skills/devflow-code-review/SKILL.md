...
description: Use when devflow-test-checker has passed and the C/C++ code change must be independently audited for correctness / SOA boundary / memory / concurrency / real-time / error-handling / coding-standard compliance / static-analysis hygiene before completion gate, when a reviewer subagent is dispatched to perform code inspection, or when code-review returned 需修改 in a previous round. Not for writing or fixing code (→ devflow-tdd-implementation), not for evaluating test effectiveness (→ devflow-test-checker), not for stage / route confusion (→ devflow-router).
---
...
- `devflow-test-checker` verdict = `通过`，需独立审 C / C++ 代码
...
- 评审测试有效性 → `devflow-test-checker`
- 阶段不清 / 证据冲突 → `devflow-router`

## Hard Gates

- `devflow-test-checker` 未通过前不得进入本 review
...
- Refactor Note 中 Escalation Triggers 非 `none` 但实现节点仍指向 `devflow-test-checker` → blocked-workflow，`reroute_via_router=true`
- 否则进入步骤 2
...
Write features/<id>/reviews/code-review.md unless AGENTS.md overrides it.

### Review Boundary

Review correctness, SOA boundary, memory/resource lifetime, concurrency, realtime behavior, error handling, coding standard, static-analysis hygiene, and evidence. Component boundary changes route to devflow-router.
