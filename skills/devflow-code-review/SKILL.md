...
description: 当 devflow-test-checker 已通过且 C/C++ 代码变更需要在完成门禁前独立审查正确性、SOA 边界、内存、并发、实时性、错误处理、编码规范符合度和静态分析卫生度时使用；也用于派发评审子代理执行代码检视，或上一轮 code-review 要求修改后复审。不用于编写或修复代码、评估测试有效性，或阶段和路由混乱。
---
