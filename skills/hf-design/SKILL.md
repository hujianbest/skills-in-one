...
| NFR 检查清单（含 QAS 承接 / observability） | `references/nfr-checklist.md` | 处理非功能需求时 |
| 失败模式分析 | `references/failure-modes.md` | 分析关键路径韧性时 |
| 架构模式选择 | `references/architecture-patterns.md` | 识别架构模式时 |
| 设计文档模板（含 Phase 0 新章节） | `references/design-doc-template.md` | 编写设计文档时 |
| DDD 战略建模 | `references/ddd-strategic-modeling.md` | 进入 C4 前锁边界 / 统一语言 / Context Map |
| Event Storming | `references/event-storming.md` | spec → design 桥接，按 profile 分档 |
| 轻量 STRIDE 威胁建模 | `references/threat-model-stride.md` | Security NFR 或跨信任边界激活时 |
