...
读取：已批准规格（默认 `features/<active>/spec.md`）、feature `progress.md`（默认 `features/<active>/progress.md`）、`AGENTS.md` 路径映射，以及其中声明的项目级设计原则锚点（默认 `docs/principles/`），外加最少必要技术上下文。当 `hf-ui-design` 被激活时，也读取其最新草稿以标记 peer 依赖条目。
