...
| 「TDD 完成了，直接 `devflow-code-review`」 | TDD 完成后**必须**先派发 `devflow-test-checker`，verdict = `通过` 才能进入 code review |
...
| 「review 在父会话里顺带做一下更快」 | 内联 review 被禁止。review 必须派发独立 reviewer subagent + 对应 `agents/*-reviewer.md` 持久 |
