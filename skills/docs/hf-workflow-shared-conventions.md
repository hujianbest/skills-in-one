...
| ADR pool | `docs/adr/NNNN-<slug>.md`（仓库级顺序号 4 位、永不复用） |
| 项目原则锚点 | `docs/principles/` |
| 长期架构图景 | `docs/arc42/`（可选） |
| 源码化图 | `docs/diagrams/`（如 Structurizr DSL / PlantUML） |
| 运维资产 | `docs/runbooks/` / `docs/slo/` / `docs/postmortems/` |
| 顶层导航索引 | `docs/index.md`（手工或 finalize 时维护） |
| 用户可见变更（详细） | `docs/release-notes/vX.Y.Z.md` |
| changelog | 仓库根 `CHANGELOG.md`（Keep a Changelog 风格） |
...
feature 周期内对 `docs/` 中长期资产的修改，按以下时机划分（详见 `docs/principles/sdd-artifact-layout.md`）：

| 长期资产 | 修改时机 | 修改方式 |
|---|---|---|
| ADR (`docs/adr/`) | 设计阶段直接落 | 起草时即分配 ADR ID 写入 `docs/adr/NNNN-...md`，状态 `proposed`；评审与 `设计真人确认` 通过后翻为 `accepted`。`design.md` 通过 ID 引用 |
| 源码化图 (`docs/diagrams/`) | 设计阶段直接落 | 与 design review 一并审核 diff |
| arc42 (`docs/arc42/`) | closeout 同步 | 由 `hf-finalize` 把已批准变更应用到对应节 |
| Glossary (`docs/arc42/12_glossary.md`) | closeout 同步 | 由 `hf-finalize` 同步新术语 |
| Runbooks (`docs/runbooks/`) | closeout 同步 | feature 引入新运维关注点时由 `hf-finalize` 新增/更新 |
| SLO (`docs/slo/`) | closeout 同步 | 由 `hf-finalize` 同步 SLO 定义变化 |
| Release notes / CHANGELOG | closeout 同步 | 由 `hf-finalize` 写入 `docs/release-notes/vX.Y.Z.md` 与 `CHANGELOG.md` |
| Bug pattern catalog (`docs/bug-patterns/`) | 由 `hf-bug-patterns` 旁路触发 | 不强制每个 feature 更新 |

`closeout.md` 必须显式列出本次 closeout 同步到 `docs/` 的所有路径作为 release/docs sync 证据；缺失时 `hf-finalize` 应判 `blocked` 并回 `hf-workflow-router`。
