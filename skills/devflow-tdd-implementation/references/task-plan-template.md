# devflow 任务计划模板

使用说明：

- 默认保存路径：`features/<工作项ID>-<slug>/tasks.md`。
- tasks 是 AR 设计到 TDD 执行的索引层，不是测试设计层；不得新增 AR 设计中没有的测试用例或业务事实。

## 元数据

- 工作项类型:
- 工作项 ID:
- 所属组件:
- 来源需求:
- 来源 AR 设计:
- 来源 AR 设计评审:
- 任务计划 Owner:
- 最后更新:

## 计划边界

- 范围内:
- 范围外:
- 假设:
- 阻塞项:

## 里程碑

| 里程碑 ID | 目标 | 包含任务 | 退出标准 | 备注 |
|---|---|---|---|---|
| M1 |  |  |  |  |

## 任务拆解

### T1 - <任务标题>

- 目标:
- 验收条件:
- 文件:
  - 
- 覆盖需求:
  - 
- Change Type / Existing Behavior Baseline:
  -
- AR 设计锚点:
  - 
- 测试设计用例 ID:
  - 
- 依赖:
  - 
- 验证方式:
  - 
- 完成定义:
  - 
- 预期证据路径:
  - 
- 实现派发:
  - 模式: controller-direct / implementer-subagent
  - 允许修改文件:
  - 范围外文件:
  - 上下文包:
- 备注 / 假设:
  - 

## 依赖关系

| 任务 ID | 依赖 | 就绪条件 | 可并行任务 | 备注 |
|---|---|---|---|---|
| T1 | - | tasks-review 通过 |  |  |

## 追溯关系

| 需求行 | Change Type | Existing Behavior / Baseline | AR 设计章节 | 测试设计用例 ID | 任务 ID | 计划证据 |
|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |

## 验证策略

| 任务 ID | 验证命令 / 检查 | 预期结果 | 证据路径 | 实现上下文说明 |
|---|---|---|---|---|
| T1 |  |  |  |  |

## 风险与待确认项

| ID | 风险 / 待确认项 | Owner | 是否阻塞 | 处理方式 |
|---|---|---|---|---|
| R1 |  |  | yes / no |  |

## TDD 前置检查就绪

- [ ] 每个关键任务具备验收条件、文件、验证方式、完成定义
- [ ] 每个任务回指需求行、Change Type / Existing Behavior Baseline、AR 设计章节、测试设计用例 ID
- [ ] `modify` / `remove` 任务包含回归 / 删除语义验证，不只验证新目标行为
- [ ] 依赖关系无环
- [ ] 已明确首个 next-ready task 或标出阻塞原因
- [ ] 未新增测试设计事实
