...
Large audits exceed a single context window — a multi-hour overnight run will likely span several cloud-agent invocations. **Use `scripts/run_audit.py` and follow `references/long-running-audits.md`**; do not invent ad-hoc batching.

Iron rule: **checkpoint after EVERY unit, not every batch.** The conductor writes per-unit findings + state atomically, so a power-cut / context-overflow / VM restart loses at most one in-progress unit. Batched checkpoints (e.g. every 20 units) are forbidden — they make the worst-case loss unbounded.

Operational summary:

- `run_audit.py init …` → builds `audit/units.jsonl` + `audit/state.json` once.
- Per-unit loop: `next` → review → `record` (atomic write to `audit/findings/<unit>.json`).
- Per-finding loop (Pass 3.5): `next-verdict` → subagent → `record-verdict`.
- Rolling `audit/partial_reports/bug_report-<ts>.xlsx` every 50 units / 30 min.
- `finalize` produces `audit/bug_report.xlsx`.
- `audit/HOWTO_RESUME.md` (auto-generated by `init`) tells a fresh agent how to pick up.

A subagent dispatch that fails or times out is recorded as `failed` / `timeout` and the queue continues — no single unit may stall the whole audit.
