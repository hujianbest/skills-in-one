...
| 检查测试是否真有效 | [`devflow-test-checker`](skills/devflow-test-checker/SKILL.md) | TDD 后测试有效性独立审查 |
...
评审节点都由 `devflow-router` 派发**独立 subagent**，subagent 以对应的 `devflow-*-review`（或 `devflow-test-checker` / `devflow-code-review`）skill 作为 system prompt。Reviewer subagent 只读被评审工件并返回结构化 verdict，不修改工件。
...
- review 节点保持独立 reviewer subagent：spec review、component design review、AR design review、test checker、code review。
