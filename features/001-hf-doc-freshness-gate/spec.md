...
| HYP-004 | `lightweight` profile 下 gate 仍能被压缩到 ≤ 5 行 checklist + ≤ 5 分钟人工耗时（**U1**） | Usability | 若 lightweight 退化为跳过，profile 分级承诺被破坏 | **high** | ✅ **fully closed by dogfooding dry run on 2026-04-23**：实测 ~2.5 分钟 + 实例化 verdict 文件 ~25 行（远低于 ≤ 5 分钟 / ≤ 30 行阈值）；详见 `features/001-hf-doc-freshness-gate/evidence/dry-run-T-NFR-002-lightweight-time.md` | 否 |
