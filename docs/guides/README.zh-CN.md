...
[English (root README)](../../README.md) | [中文（精简版根 README）](../../README.zh-CN.md) | **本页：中文深度指南**

> v1.0.0 起，本仓库品牌统一为 **DevFlow**（与仓库名一致）。本指南是 `docs/guides/` 下的深度版本；面向"5 分钟上手"的精简介绍见根目录 [`README.zh-CN.md`](../../README.zh-CN.md)。

**DevFlow** 是一套面向 AI Coding 开发阶段的 skill family，用来把团队已经接受的 SR / AR / DTS / CHANGE work item 推进到需求澄清、设计、TDD 实现、独立评审、完成门禁和收尾。
...
- **Review separation**：authoring、implementation、test review、code review、completion gate 保持角色分离。
...
flowchart TD
  ...
  testCheck -->|"是"| testReview[devflow-test-review]
...
```text
specify -> spec review -> AR design -> AR design review -> TDD -> test review -> code review -> completion gate -> finalize
```
...
```text
problem fix -> root cause -> minimal fix boundary -> TDD -> test review -> code review -> completion gate -> finalize
```
...
| `devflow-test-review` | TDD 完成后审查已落地测试是否真正有效。 |
| `devflow-code-review` | 审查代码质量、架构边界、嵌入式 C/C++ 风险和可维护性。 |
| `devflow-completion-gate` | 消费已有证据并判断是否完成，不制造缺失证据。 |

因此，TDD 完成后不能直接进入 code review，必须先经过 `devflow-test-review`。Code review 通过后也不能直接宣布完成，必须经过 `devflow-completion-gate`。
...
implementer subagent 返回 `DONE`、`DONE_WITH_CONCERNS`、`NEEDS_CONTEXT` 或 `BLOCKED`。controller 将结果记录到 `task-board.md` / `implementation-log.md`，再派发 test review 和 code review。implementer 的 self-review 不能替代独立评审。
...
.
  README.md
  README.zh-CN.md
  AGENTS.md                  # OpenCode 上 DevFlow 的硬约束（v1.0.0 新增）
  LICENSE                    # MIT（v1.0.0 新增）
  CONTRIBUTING.md            # v1.0.0 新增
  CHANGELOG.md               # v1.0.0 新增
  docs/
    guides/
      README.zh-CN.md        # 本指南
      opencode-setup.md      # OpenCode 安装与接入说明（v1.0.0 新增）
    ...
    principles/
      ...
      06 evals-format.md     # v1.0.0 新增：evals/ 目录与 evals.json 字段契约
  skills/
    using-devflow/
    devflow-router/          # 含 evals/（v1.0.0 新增，8 个 misuse 场景）
    devflow-specify/
    devflow-spec-review/
    devflow-component-design/
    devflow-component-design-review/
    devflow-ar-design/
    devflow-ar-design-review/
    devflow-tdd-implementation/  # 含 evals/（8 个场景）
    devflow-test-review/        # 含 evals/（7 个场景）
    devflow-code-review/
    devflow-completion-gate/     # 含 evals/（8 个场景）
    devflow-finalize/
    devflow-problem-fix/
```

每个 skill 都按可独立使用来组织。共享约定和模板已经内化到各自的 `SKILL.md` 或本地 `references/` 目录中，以支持按需安装和使用。高风险 skill 额外带 `evals/` 目录，枚举它们必须拒绝的 misuse 场景；格式见 [`docs/principles/06 evals-format.md`](../principles/06%20evals-format.md)。

OpenCode 接入步骤见 [`docs/guides/opencode-setup.md`](opencode-setup.md)；agent 必须遵守的硬约束见仓库根 [`AGENTS.md`](../../AGENTS.md)。
...
### 为什么 TDD 后还要 `devflow-test-review`？

TDD 产生测试，但不自动证明测试有效。`devflow-test-review` 审查测试是否覆盖关键行为、边界、异常路径和嵌入式风险，断言是否真正证明行为，而不只是证明代码被调用。
...
详见仓库根目录 [`CHANGELOG.md`](../../CHANGELOG.md)。本指南文件本身的演进合并到 v1.0.0 条目中：

### v1.0.0

- 新增本指南（`docs/guides/README.zh-CN.md`），覆盖 AI Coding 痛点、DevFlow 价值主张、安装准备、快速开始、使用示例、交付结果、FAQ。
- 与 v1.0.0 release 同步：品牌统一为 **DevFlow**（与仓库名一致），仓库结构补齐 `AGENTS.md` / `LICENSE` / `CONTRIBUTING.md` / `CHANGELOG.md` / `docs/principles/06 evals-format.md` / 各高风险 skill 的 `evals/`。
- 将当前 DevFlow skill family 路径统一为 `skills/`，将原则文档路径统一为 `docs/principles/`。
- 为普通 AR、Component Impact、DTS / Hotfix 和续跑已有工作项补充案例骨架与截图占位。
- 保留当前关键架构约束：artifact-first、router 路由、Design Options checkpoint、TDD 后 test review、独立 code review 和 completion gate。
