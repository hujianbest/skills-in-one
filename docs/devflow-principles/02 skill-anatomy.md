...
| Test Checker | `devflow-test-checker` | TDD 后测试用例有效性、覆盖性、可维护性审查 |
...
- TDD 完成后未经过 `devflow-test-checker` 却进入代码检视。
- `devflow-test-checker` 试图补写测试或修改生产代码。
...
| TDD 后不做测试有效性审查 | 测试可能只是跑通但覆盖不足 | 路由到 `devflow-test-checker` |
| `devflow-test-checker` 补写测试或改生产代码 | reviewer 角色越权 | 返回 findings，让实现节点回修 |
