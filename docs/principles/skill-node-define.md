...
2. 声明它使用什么方法完成该职责。
3. 能把方法落成具体 workflow todo list。
4. 能读取权威上游工件，而不是依赖对话记忆。
5. 能产出或评审明确工件、record 或 evidence。
6. 能给出唯一下一步，或明确阻塞并回到 router。
7. 能被 `hf-workflow-router` 根据磁盘证据重新进入。

简写为：

```text
HF flow node = skill + responsibility + methods + workflow todos + artifacts + evidence + handoff
```

## Four Contracts

每个 HF node skill 必须同时满足四类 contract。
...
这四类 contract 不应彼此替代。只写步骤而不写方法，会让节点退化成 prompt checklist；只写方法而不写 todo，会让节点不可执行；只写下一步而不写证据，会让 workflow 不可恢复。
...
| Step | Todo | Method | Input | Output | Stop / Continue Rule |
|---|---|---|---|---|---|
| 1 | Precheck 是否可进入本节点 | Evidence-Based Decision Making | progress、上游 record、用户请求 | 进入 / 阻塞判断 | 证据冲突则回 router |
| 2 | 读取最少必要上游工件 | Traceability Reading | spec / design / tasks / reviews | 上下文摘要 | 发现缺失或冲突则阻塞 |
| 3 | 执行节点核心工作 | 节点声明的方法论 | 上游输入 | 新工件、review record 或 evidence | 达不到完成条件则继续修订 |
| 4 | 写入结果 | Artifact Contract | 本轮结论 | artifact path / record_path | 写不出记录则不得 handoff |
| 5 | 交接下一步 | Transition Contract | verdict / result | next_action_or_recommended_skill | 下一步不唯一则回 router |
...
1. 该节点为什么需要这些方法？
2. 这些方法解决节点职责中的什么风险？
3. 方法如何映射到 workflow todo？
4. 方法如何影响输出工件结构？
5. 方法如何影响 review、gate 或 verification 标准？
6. 哪些方法是 hard rule，哪些只是辅助参考？
