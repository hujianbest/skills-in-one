...
这份协议说明 `using-mdc-workflow` 与各上游产出 skill 在遇到 review 节点时，如何把评审动作派发给独立 reviewer subagent，而不是在父会话里内联执行 review。
...
4. reviewer subagent 在 fresh context 中读取对应 review skill 与最小必要工件。
...
| --- | --- |
| `mdc-review`(spec) | `mdc-review` (review_type=spec) |
| `mdc-review`(arch-design) | `mdc-review` (review_type=arch-design) |
| `mdc-review`(ar-design) | `mdc-review` (review_type=ar-design) |
| `mdc-review`(tasks) | `mdc-review` (review_type=tasks) |
| `mdc-code-review` | `mdc-code-review` |
| `mdc-test-checker` | `mdc-test-checker` |
...
```json
{
  "review_type": "spec|arch-design|ar-design|tasks|code|test",
  "review_skill": "mdc-review|mdc-code-review|mdc-test-checker",
  "topic": "本次评审主题",
  "artifact_paths": [
    "被检视交付件路径"
  ],
...
- 判断当前是否应进入 review 节点
- 选择正确的 review skill 与 review_type
- 组装最小 review request
...
- 读取对应 review skill（`mdc-review`、`mdc-code-review` 或 `mdc-test-checker`）
...
- `mdc-review`(spec) — `规格真人确认`
- `mdc-review`(arch-design) — `架构设计真人确认`
- `mdc-review`(ar-design) — `AR设计真人确认`
- `mdc-review`(tasks) — `任务真人确认`
...
则 reviewer 应在返回摘要中明确要求父会话经 `using-mdc-workflow` 重编排，而不是让下游 skill 自行补位推进。
