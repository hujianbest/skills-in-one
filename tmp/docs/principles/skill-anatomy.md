# Skill Anatomy — Skill 写作原则

- 定位: 项目级原则文档，定义 MDC workflow skills 的目标态写法。

## 定位

本文定义 MDC workflow skills 的目标态写法。
...
这是 skill 的通用类型，不是 MDC workflow 节点角色：
...
MDC workflow skill 大多是 `Technique + Pattern` 的混合体；当某个节点依赖 rubric、模板、协议或映射表时，再引入 `Reference` 层。

## MDC 节点角色

这是 MDC family 内部的 workflow 角色：

| 角色           | 代表 skill                                                        | 写作重心                                                                      |
| -------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Public Entry   | `using-mdc-workflow`                                              | 入口判断、route-first vs direct invoke、runtime routing（MDC 路由合并在其中） |
| Authoring      | `mdc-specify` / `mdc-arch-design` / `mdc-ar-design` / `mdc-tasks` | 起草、回修、自检、评审 handoff                                                |
| Review         | `mdc-review` / `mdc-test-checker` / `mdc-code-review`             | precheck、rubric、findings、verdict                                           |
| Implementation | `mdc-test-driven-dev`                                             | 唯一实现入口、TDD、fresh evidence、交接块                                     |
| Gate           | `mdc-finalize`                                                    | evidence bundle、门禁结论、唯一下一步                                         |
| Finalize       | `mdc-finalize`                                                    | closeout、状态闭合、release notes、handoff pack                               |
...
建议语义使用等价于 `Use when ... / Not for ...` 的写法。MDC 可写中文，但语义必须是分类器语义，而不是摘要语义。

```yaml
# ❌ BAD: 摘要了 workflow
description: Use when routing MDC workflow - read evidence, decide profile, dispatch reviewer, continue execution
...
每个 MDC skill 至少要说明最容易混淆的相邻节点。常见邻接关系：

| 当前 skill | 至少要区分谁 |
|---|---|
| `using-mdc-workflow` | 无（它是公共入口） |
| `mdc-specify` | `mdc-arch-design` / `mdc-ar-design` / `mdc-review(review_type=spec)` |
| `mdc-arch-design` / `mdc-ar-design` | `mdc-arch-design` vs `mdc-ar-design` / `mdc-review(review_type=arch-design)` / `mdc-review(review_type=ar-design)` |
| `mdc-tasks` | `mdc-arch-design` / `mdc-ar-design` / `mdc-review(review_type=tasks)` |
| `mdc-test-driven-dev` | `mdc-test-checker` / `mdc-review` / `mdc-finalize` |
| `mdc-finalize` | `mdc-review` / `mdc-finalize` closeout 分支 |
  ...
  "evals": [
    {
      ...
      "prompt": "<模拟用户请求>",
      "expected_behavior": "应拒绝并 re-route 到 mdc-specify",
      "assertions": [
        "输出包含 reroute 指令",
        "输出不包含设计内容"
      ],
...
MDC 的目标态 skill anatomy，是把 `SKILL.md` 写成：
- **分类器式的 description**
- **清晰的触发与边界**
- **编号步骤和明确的 reroute 路径**
- **500 行以内的主文件**

而把长资料下沉到 `references/`，把行为 contract 写进 `evals/`，把共享约定上收到 `docs/`。

每写一个 skill，先问自己："三年后的另一个 agent，能凭这份描述和边界，正确使用我吗？"
