# MDC Requirement Authoring Contract

## Purpose

本文定义 `mdc-specify` 在起草规格时对单条需求的最小写法契约。它不是固定模板，而是要求无论项目模板如何变化，核心需求都必须保持同一组可回读语义。

## Minimum Rule

对于 non-trivial 的规格，核心 `FR` / `NFR` 不应只写成松散段落。每条需求至少应具备一组稳定字段，方便 `mdc-review(review_type=spec)` 做冷读、判定和回修。
...
- 正式 review 的检查维度和 finding 分类，见 `mdc-review/references/spec-review-rubric.md`
