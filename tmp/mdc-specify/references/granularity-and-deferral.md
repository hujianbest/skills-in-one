# MDC Granularity And Deferral

## Purpose

本文定义 `mdc-specify` 在规格阶段如何判断需求是否过大、是否跨了多个发布轮次，以及哪些内容应该进入延后清单而不是继续塞进当前轮范围。
...
- 已被澄清为真实需求，但当前轮不做
- 已有初步优先级和来源
- 后续很可能通过 `using-mdc-workflow` 重新路由回收
...
|---|---|---|---|---|---|---|
| FR-007 | 管理员导出审批报表 | Could | 不属于当前 MVP 主流程 | 无当前轮硬依赖 | 当审批主流程与留存规则稳定后再回收 | using-mdc-workflow 重新路由 |
```
...
deferred backlog 的 canonical 回收入口是通过 `using-mdc-workflow` 重新判断阶段并路由。它不是自动下游节点，也不是当前规格阶段的替代完成条件。
