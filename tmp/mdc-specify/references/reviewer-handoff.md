...
草稿准备好后，不要在父会话里内联执行 `mdc-review(review_type=spec)`。正确做法是：

1. 将规格草稿保存到约定路径
2. 若存在 deferred backlog，一并保存到约定路径
3. 组装最小 spec review request
4. 启动独立 reviewer subagent，并在该 subagent 中调用 `mdc-review(review_type=spec)`
...
| `需修改` | 携带关键 findings 回到 mdc-specify 修订 |
| `阻塞` + `reroute_via_router=true` | 回到 `using-mdc-workflow` 重编排 |
| 其他 `阻塞` | 携带关键 findings 回到 mdc-specify 补条件或修订 |
...
- **route/stage/profile 冲突**：不在本节点硬修，交回 `using-mdc-workflow`

## 禁止项

- 不要在 `mdc-specify` 阶段请求 approval step；只发生在 `mdc-review(review_type=spec)` 返回"通过"之后
