...
- 这是 `mdc` 中用于 task-to-task 自动推进的可选 coordination artifact 模板。
...
- 若 task board 与已批准任务计划冲突，或无法唯一判断 `next-ready task`，应停止自动推进并回到 `using-mdc-workflow`。
...
- Conflict Policy:
  - 若存在多个同等候选，或依赖 / 状态冲突，则回到 `using-mdc-workflow`
- Ready Semantics:
- Done Semantics:
...
- `done`: 当前任务已完成实现质量链，并通过 `mdc-finalize`
