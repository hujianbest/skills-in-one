...
- [ ] 所有可能失败的操作是否有错误处理
- [ ] 错误码使用是否正确（HI_SUCCESS/HI_FAILURE）
- [ ] 错误日志是否完整（包含上下文信息）
- [ ] 异常安全是否考虑（资源泄漏风险）

### 1.3 类型安全

- [ ] 是否使用项目特定类型（hi_u8, hi_s32, hi_u32等）
- [ ] 类型转换是否安全（避免隐式转换）
- [ ] 指针使用是否安全（空指针检查）
- [ ] 是否使用无符号类型后缀（U, ULL）
...
- [ ] TearDown是否调用 `GlobalMockObject::verify()`
- [ ] Mock设置是否合理
...
### 4.1 命名规范
- [ ] 是否符合(PascalCase/camelCase/snake_case）
- [ ] 是否有版权信息
...
**问题模式：** 使用裸指针new/delete
```cpp
// ❌ Bad: 异常不安全
uint8_t* buffer = new uint8_t[1024];
process(buffer);  // 如果这里抛异常，buffer泄漏
delete[] buffer;
