# MDC Spec Review Rubric

## Purpose

本文定义 `mdc-review(review_type=spec)` 的正式审查维度，使 reviewer 不只是"看起来像规格就放行"，而是按统一规则判断规格是否真的足以成为 `mdc-ar-design` 的稳定输入。
...
| `C4` | Template alignment | 文档遵循当前项目模板或 `AGENTS.md` 约定；若没有模板，至少符合 MDC 默认骨架 |
...
为了让 `mdc-specify` 能定向回修，建议在 review 记录中用以下标签格式：
